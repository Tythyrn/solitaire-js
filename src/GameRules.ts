...
export default class GameRules {
    ...
    constructor(game: Game){
        this.game = game;
        this.previousStates = new Set<string>();
    }
    ...
    private moveCardsFromStockToFoundation(): boolean {
    ...
    private moveCardsfromTableauToFoundation(): boolean {
    ...
    private moveCardsBetweenTableaus(): boolean {
        for(let i = this.game.tableauPiles.length - 1; i >= 0; i--) {
            const currentTableau = this.game.tableauPiles[i]
    ...
    private moveCardsFromStockToTableaus(): boolean {
        this.game.stockPile.refillFromWaste();
    ...
    private moveCardsFromFoundationToTableau(): boolean {
        for (const foundationPile of this.game.foundationPiles) {
